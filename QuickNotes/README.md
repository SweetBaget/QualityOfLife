...
скрипт**, поэтому окно, панель задач и переключение Alt+Tab показывают значок —
при запуске как одного `QuickNotes.ps1`, так и через лаунчер. Приложение также
регистрирует собственную идентичность на панели задач (AppUserModelID), чтобы
кнопка не группировалась с PowerShell и не наследовала его значок.

### Ярлык со значком

Сами файлы `.ps1`/`.bat` собственного значка показать не могут (это ограничение
Windows), поэтому для рабочего стола и панели задач используйте ярлык:

- **Автоматически:** запустите `Create Desktop Shortcut.ps1` (правый клик →
  **Выполнить с помощью PowerShell**) — на рабочем столе появится ярлык
  «Быстрые заметки» с иконкой. Его можно перетащить/закрепить на панель задач.
- **Вручную:** создайте ярлык к `Start QuickNotes.bat` → Свойства ярлыка →
  **Сменить значок…** → выберите `AppIcon.ico`.

### Если значок на панели задач не обновился

1. Убедитесь, что запускается **обновлённый** `QuickNotes.ps1` (внутри есть
   секция «ИКОНКА ПРИЛОЖЕНИЯ» и `AppUserModelID`) — скопируйте всю папку
   заново.
2. Если приложение было **закреплено** на панели задач раньше — открепите и
   закрепите заново через новый ярлык: закрепление сохраняет старый значок.
3. Сбросьте кэш значков Windows: `Win+R` → `ie4uinit.exe -show` → Enter
   (или пересоздайте ярлык; в упорных случаях — перезапуск проводника:
   Диспетчер задач → «Проводник» → Перезапустить).
